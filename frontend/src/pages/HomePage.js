...
const HomePage = () => {
  ...
  const stats = [
    ...
    { label: 'Countries Covered', value: getUniqueCountries().length, icon: Users },
    { label: 'Government Schemes', value: loans.filter(l => l.lenderType === 'government').length, icon: Shield },
    { label: 'Bank & Private Lenders', value: loans.filter(l => l.lenderType === 'bank' || l.lenderType === 'private').length, icon: Award }
  ];
